...
function get(path, token) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('data', c => body += c);
      res.on('end', () => {
        try { resolve(JSON.parse(body)); } catch(e) { resolve({ raw: body.substring(0,200) }); }
      });
    });
    req.on('error', reject);
    req.end();
...
async function main() {
  ...
  const adminPasswords = ['admin123', 'password123', 'Admin@123', 'Admin123!', 'super_admin'];
  
  console.log('=== ADMIN LOGIN TESTS ===');
  for (const pw of adminPasswords) {
    const r = await post('/api/auth/login', { email: 'admin@zevio.com', password: pw });
    console.log(`  admin@zevio.com / ${pw}: ${r.success ? 'SUCCESS ('+r.data.user.role+')' : r.message}`);
    if (r.success) {
      ...
      console.log(`  Refresh: ${refreshResp.success ? 'SUCCESS' : 'FAILED - '+refreshResp.message}`);
      
      if (refreshResp.success) {
        const newAt = refreshResp.data.accessToken;
        const newRt = refreshResp.data.refreshToken;
        console.log(`  New AccessToken: ${newAt ? newAt.substring(0,40)+'...' : 'NOT RETURNED'}`);
        console.log(`  New RefreshToken: ${newRt ? newRt.substring(0,40)+'...' : 'NOT RETURNED!'}`);
        
        // Test old token is revoked
        console.log('\n=== OLD TOKEN REVOCATION TEST ===');
        const oldResp = await post('/api/auth/refresh', { refreshToken: rt });
        console.log(`  Old token reuse: ${oldResp.success ? 'STILL WORKS (BAD!)' : 'REVOKED (CORRECT) - '+oldResp.message}`);
        
        // Test new token works
        console.log('\n=== NEW TOKEN WORKS TEST ===');
        if (newRt) {
          const newResp = await post('/api/auth/refresh', { refreshToken: newRt });
          console.log(`  New token refresh: ${newResp.success ? 'WORKS (CORRECT)' : 'FAILED - '+newResp.message}`);
          ...
          for (const ep of endpoints) {
            ...
            console.log(`  GET ${ep}: ${r.success ? 'OK' : 'FAIL - '+r.message}`);
          }
          ...
          if (propsResp.success) {
            ...
            for (const p of props.slice(0, 5)) {
              console.log(`  ${p.title}: price=${p.price_per_night}, original_price=${p.original_price}, status=${p.status}`);
            }
          }
        }
      }
  ...
  for (const pw of vendorPasswords) {
    const r = await post('/api/auth/login', { email: 'kruthiksatish9@gmail.com', password: pw });
    console.log(`  kruthiksatish9@gmail.com / ${pw}: ${r.success ? 'SUCCESS ('+r.data.user.role+')' : r.message}`);
    if (r.success) {
      const vat = r.data.accessToken;
      
      console.log('\n=== VENDOR AUTHENTICATED ENDPOINTS ===');
      const vResp = await get('/api/vendor/properties', vat);
      console.log(`  GET /api/vendor/properties: ${vResp.success ? 'OK - '+((vResp.data.properties||vResp.data).length)+' properties' : 'FAIL - '+vResp.message}`);
      
      // Check original_price in vendor properties
      if (vResp.success) {
        const vProps = vResp.data.properties || vResp.data;
        for (const p of vProps.slice(0, 3)) {
          console.log(`  ${p.title}: price=${p.price_per_night}, original_price=${p.original_price}`);
        }
      }
      break;
    }
