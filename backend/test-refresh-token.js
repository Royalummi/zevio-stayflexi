...
async function main() {
  // 1. Login
  console.log('1. Login...');
  const login = await post('/api/auth/login', { email: 'admin@zevio.com', password: 'password123' });
  if (!login.success) { console.log('Login FAILED:', login.message); return; }
  
  const rt1 = login.data.refreshToken;
  console.log('   RT1 hash:', crypto.createHash('sha256').update(rt1).digest('hex').substring(0, 20) + '...');
  ...
  console.log('   Result:', ref1.success ? 'SUCCESS' : 'FAIL - ' + ref1.message);
  
  if (!ref1.success) return;
  const rt2 = ref1.data.refreshToken;
  console.log('   RT2 hash:', crypto.createHash('sha256').update(rt2).digest('hex').substring(0, 20) + '...');
  ...
  console.log('   Result:', ref2.success ? 'STILL WORKS (BAD!' : 'REVOKED (CORRECT) - ' + ref2.message);
  
  // 4. Use RT2 - should work
  console.log('4. Use RT2...');
  const ref3 = await post('/api/auth/refresh', { refreshToken: rt2 });
  console.log('   Result:', ref3.success ? 'SUCCESS' : 'FAIL - ' + ref3.message);
  ...
  if (ref3.success) {
    ...
    console.log('   Result:', ref4.success ? 'STILL WORKS (BAD!)' : 'REVOKED (CORRECT)');
  }
