...
async function testAdminLogin() {
  ...
  log(ok, 'Admin login', `status=${res.status}, role=${res.body.data?.user?.role || 'N/A'}`);
...
async function testVendorLogin() {
  ...
  const ok = res.status === 200 && res.body.success;
  log(ok, 'Vendor login', `status=${res.status}, role=${res.body.data?.user?.role || res.body.message || 'N/A'}`);
  return ok ? res.body.data : null;
}

async function testRefreshTokenRotation(authData) {
  console.log('\n=== 3. REFRESH TOKEN ROTATION ===');
  if (!authData) { log(false, 'Refresh token', 'No auth data'); return null; }
  ...
  log(different, 'RT2 is different from RT1', different ? 'Unique tokens' : 'SAME TOKEN (broken)');
  ...
  log(revoked, 'Old RT1 is revoked', `status=${res2.status}, msg=${res2.body.message || 'N/A'}`);
  ...
  return { accessToken: res3.status === 200 ? res3.body.data.accessToken : at2, refreshToken: res3.status === 200 ? res3.body.data.refreshToken : rt2 };
}

async function testAdminBanners(token) {
  console.log('\n=== 4. ADMIN BANNERS ===');
  if (!token) { log(false, 'Admin banners', 'No token'); return; }
  ...
  if (res.status === 200) {
    ...
    log(hasBanners, 'Response has banners array', `type=${typeof data}, keys=${data ? Object.keys(data) : 'N/A'}`);
  } else {
    log(false, 'Banners response structure', `msg=${res.body.message || JSON.stringify(res.body).substring(0, 100)}`);
  }

  // Public banners
  const pub = await req('GET', '/api/banners/active');
  log(pub.status === 200, 'GET /api/banners/active (public)', `status=${pub.status}`);
}

async function testAdminCreateProperty(token) {
  console.log('\n=== 5. ADMIN CREATE PROPERTY (with original_price) ===');
  if (!token) { log(false, 'Admin create property', 'No token'); return null; }
  ...
  if (!ok) {
    log(false, 'Property creation response', `msg=${res.body.message || JSON.stringify(res.body).substring(0, 200)}`);
    return null;
  }
  ...
  if (propId) {
    const det = await req('GET', `/api/admin/properties/${propId}`, null, token);
    if (det.status === 200) {
      const prop = det.body.data;
      const origPrice = prop?.pricing?.original_price || prop?.original_price;
      log(origPrice == 20000, 'original_price saved correctly', `got=${origPrice}, expected=20000`);
    } else {
  ...
  const list = await req('GET', '/api/admin/properties?page=1&limit=5', null, token);
  if (list.status === 200) {
    const props = list.body.data?.properties || list.body.data || [];
    const found = props.find(p => p.id === propId);
    if (found) {
      log('original_price' in found, 'original_price in list response', `value=${found.original_price}`);
    } else {
...
async function testVendorCreateProperty(token) {
  ...
  if (!token) { log(false, 'Vendor create property', 'No token'); return null; }
  ...
  if (!ok) {
    log(false, 'Vendor property response', `msg=${res.body.message || JSON.stringify(res.body).substring(0, 200)}`);
    return null;
  }
  ...
  if (propId) {
    const det = await req('GET', `/api/vendor/properties/${propId}`, null, token);
    if (det.status === 200) {
      const prop = det.body.data?.property || det.body.data;
      const origPrice = prop?.original_price;
      log(origPrice == 12000, 'Vendor original_price saved correctly', `got=${origPrice}, expected=12000`);
    } else {
...
async function testCleanup(adminToken, adminPropId, vendorPropId) {
  ...
  if (adminPropId && adminToken) {
    const r = await req('DELETE', `/api/admin/properties/${adminPropId}`, null, adminToken);
    log(r.status === 200 || r.status === 204, 'Delete admin test property', `status=${r.status}`);
  }
  if (vendorPropId && adminToken) {
    const r = await req('DELETE', `/api/admin/properties/${vendorPropId}`, null, adminToken);
    log(r.status === 200 || r.status === 204, 'Delete vendor test property', `status=${r.status}`);
  }
}
