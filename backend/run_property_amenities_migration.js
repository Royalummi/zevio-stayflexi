...
try {
  // --- Check living_area ---
  const [laCols] = await db.query(
    "SHOW COLUMNS FROM `properties` LIKE 'living_area'",
  );
  if (laCols.length === 0) {
    await db.query(
  ...
  const [aoCols] = await db.query(
    "SHOW COLUMNS FROM `amenities` LIKE 'apartment_only'",
  );
  if (aoCols.length === 0) {
    await db.query(
      "ALTER TABLE `amenities` ADD COLUMN `apartment_only` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'If 1, only show for Service Apartment properties' AFTER `is_active`",
    );
    console.log("✅ Added apartment_only column to amenities");

    // Mark Housekeeping as apartment_only
    const [hk] = await db.query(
      "UPDATE `amenities` SET `apartment_only` = 1 WHERE `name` = 'Housekeeping' AND `category` = 'service'",
    );
    console.log(
      `✅ Marked Housekeeping as apartment_only (${hk.affectedRows} row(s))`,
    );
  } else {
  ...
  console.log(
    `✅ Kitchen display_order set to 1 (${kRes.affectedRows} row(s))`,
  );
  ...
  for (const amenity of newAmenities) {
    const [exists] = await db.query(
      "SELECT id FROM `amenities` WHERE `name` = ? AND `category` = ?",
